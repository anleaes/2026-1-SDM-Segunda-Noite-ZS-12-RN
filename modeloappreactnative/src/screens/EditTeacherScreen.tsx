...
type Props = DrawerScreenProps<DrawerParamList, "EditTeacher">;

const EditTeacherScreen = ({ route, navigation }: Props) => {
  const { teacher } = route.params;

  const [name, setName] = useState(teacher.name);
  const [cpf, setCpf] = useState(teacher.cpf);
  const [email, setEmail] = useState(teacher.email);
  const [titulacao, setTitulacao] = useState<Teacher["titulacao"]>(
    teacher.titulacao,
  );
  const [registro, setRegistro] = useState(teacher.registro);
  ...
  useEffect(() => {
    ...
    setTitulacao(teacher.titulacao);
    setRegistro(teacher.registro);
  }, [teacher]);

  const handleSave = async () => {
    setSaving(true);
    try {
      await fetch(`http://localhost:8000/professor/${teacher.id}/`, {
        method: "PUT",
